...
- **Scheduled-automation infrastructure (LIVE as of 2026-07-10, DEC-0069 / CLD-00062).** All scheduled work that touches canonical Studio files runs as **local Claude Code under launchd** — one nightly chain, `~/Claude/Scheduled/nightly/cowork-nightly.sh` (`com.cowork.nightly`, 23:00 PT): deterministic transcript export + integrity lint → EOD compaction → graph refresh (absorbs the retired 23:25 agent) → reviewed git sweep + push. Replaces the dead Cowork scheduled-EOD path (headless cloud sessions have no device bridge) and the legacy `cowork-end-of-day-compaction` SKILL (retired 2026-07-10/11). Alerting is FAIL-only; FLAG-level findings are triaged by the agent at next session start (CLD-00063). This is the concrete Layer-2 binding for the Cowork remote-execution era — see the LaunchAgent note under Stage 4 below for why the *binding* does not port to Alfred.
- **Remote transcript protocol (DEC-0070, LIVE 2026-07-11).** Remote cloud-container Cowork chats transcribe to `cowork/remote_<UUID>.md`, captured **in-container** by a deterministic converter (`export-remote-transcript.py`) on a soft-close flush lifecycle (DEC-0073/0074); the nightly chain is verifier-only for them. Transcript tooling recognizes three ownership modes — `hand` / `tool-owned` / `live-deterministic` (CLD-00063). Platform mechanics: `~/Claude/memory/reference/remote-cowork-platform.md`.
