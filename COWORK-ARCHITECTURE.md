...
> **Update 2026-07-09 (DEC-0069 / CLD-00062):** the 2026-07-07 platform change moved Cowork execution to Anthropic cloud sandboxes; scheduled Cowork tasks run headless with **no device bridge** and cannot reach canonical files. Scheduled automation therefore moves to **local Claude Code via launchd** — a nightly chain (EOD compaction → transcript tooling → graph refresh → reviewed git sweep + push) replacing both the Cowork scheduled-EOD path and this document's "nightly LaunchAgent commits corpus changes" description (that agent's refresh step is absorbed; its commit step retires into the sweep). Design: `Design/code-scheduled-automation-architecture.md`. Platform mechanics: `~/Claude/memory/reference/remote-cowork-platform.md`. Sections below describing session_info-based EOD backfill or the 23:25 commit are superseded to that extent.
>
> **Update 2026-07-11 (CLD-00062 Phase 4 / CLD-00063 Phase 3): the nightly chain is LIVE.** `~/Claude/Scheduled/nightly/cowork-nightly.sh` runs under launchd `com.cowork.nightly` at 23:00 PT (installed 2026-07-09; night-2 on 2026-07-10 ran clean end-to-end — export → EOD → lint → graph refresh → reviewed git sweep + push). The old 23:25 graph LaunchAgent is retired (`.retired-CLD00062`); the legacy `cowork-end-of-day-compaction` Cowork task is neutered/retired (registration removal pending in the app UI). **Remote transcript protocol (DEC-0070):** remote cloud-container chats (`cowork/remote_<UUID>.md`) are captured **in-container** by a deterministic converter (`export-remote-transcript.py`) with a soft-close flush lifecycle (DEC-0073/0074); the nightly is **verifier-only** for them (no Mac-side source). Transcript tooling now recognizes three ownership modes — `hand` / `tool-owned` / `live-deterministic` (CLD-00063). Wherever the sections below still describe session_info EOD backfill or the 23:25 commit as current, the live reality is this chain.
